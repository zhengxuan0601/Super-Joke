import { getRandom, newRandomId } from '@/lib/util'
const colorList = ['#2080f7', '#0f0d39', '#c728cb', '#e8a3ea', '#f2d61f', '#622725', '#f40dc4', '#0df4c7']
/**
 * 小球构造函数
 * @param {x} 小球横坐标点
 * @param {y} 小球纵坐标点
...
function Ball ({ width, height, x, y, r, color, speed }) {
  this.width = width
  this.height = height
  this.x = x || Math.floor(getRandom(0, width))
  this.y = y || Math.floor(getRandom(0, height))
  this.r = r || 0.6
  this.color = color || colorList[Math.floor(getRandom(0, 8))]
  this.speed = speed || 0.5
  this.id = newRandomId()
  this.randomAngel = getRandom(0, 360) * Math.PI / 180
  this.vx = getRandom(-0.6, 0.6)
  this.vy = getRandom(-4, -3)
  this.g = 0.1
...
function Line (moveTo, lineTo, color) {
  this.moveTo = moveTo
  this.lineTo = lineTo
  this.color = color || colorList[Math.floor(getRandom(1, 3))]
